...
{  
   split(rgb,bgr);
   inRange(bgr[2],threshold_value_white_min+230,threshold_value_white_max,binary_yellow[0]);//shan
   inRange(bgr[1],threshold_value_white_min+230,threshold_value_white_max,binary_yellow[1]);//shan
   cv::Mat src;
   ...
   //imshow("binary_yellow", binary_white);//shan
}
void binariser_red(int,void*)
{  split(rgb,bgr);
   //threshold(bgr[2],binary_red,threshold_value_red,threshold_max,0);
   inRange(bgr[2],threshold_value_red_min+230,threshold_value_red_max,binary_red);
   inRange(bgr[1],threshold_value_red_min+230,threshold_value_red_max,binary_green);
...
{  split(rgb,bgr);
   ...
   inRange(bgr[0],threshold_value_blue_min+230,threshold_value_blue_max,binary_blue);//shan
...
{           
            ...
            //circle(contours_image, pt,4, Scalar (0,255,0), CV_FILLED, 2,0); // to check the 0,0 coordinate//shan
            ...
            for(int i=0; i<contours.size(); i++)
             ...
             { idx=id[m];
               ...
               //drawContours(area_image,contours,id[m],Scalar(0,255,0),2);//, CV_FILLED);//shan
               ...
               if (counter_tf>20)             
               {
                ...
                {       
                       ...
                       //circle(contours_image, mc[idx],1, Scalar (0,255,0), CV_FILLED, 1,0);//shan
                       ...
                       {putText(contours_image,truck_id,mc[idx], 1, 0.9, Scalar (0,0,0), 1);}
                       ...
                       if (id_t==4)
                          ...
                          {  
                             ...
                             pt_right.x=right_x;
                             pt_right.y=right_y;
                             //circle(contours_image, pt_right,3, Scalar (0,0,255), CV_FILLED, 2,0);//shan 
    ...
    while (key != 'q') {
        ...
        if (zed.grab(runtime_parameters) == SUCCESS) {
            ...
            */
            ...
            createTrackbar("yel_min+230", "area", &threshold_value_white_min, threshold_max-230, binariser_white);//shan
            binariser_white(0,0);
	    ...
	    threshold_value_red_max=255;
            createTrackbar("red_min+230", "area", &threshold_value_red_min, threshold_max-230, binariser_red);
            binariser_red(0,0);
           ...
           contour_finder(copy_r,identity_r,2, counter_t);
            ...
            createTrackbar("blu_min+230", "binary_blue", &threshold_value_blue_min, threshold_max-230, binariser_blue);//shan
            binariser_blue(0,0);
