...
int id[100];
double gsd= 0.0372 ; //.028672: gsd for altitude =20, resaolution= 10800p  in meter     //.021505: gsd for altitude =15, resaolution= 10800p  in meter  //  .0075:gsd for altitude =7, resaolution= 720p  in meter
float frame_h=1080;  // resolution of the frame-hight
float frame_w=1920;
float resize_ratio=.5;
...
{  
   split(rgb,bgr);
   inRange(bgr[2],threshold_value_white_min,threshold_value_white_max,binary_yellow[0]);
   inRange(bgr[1],threshold_value_white_min,threshold_value_white_max,binary_yellow[1]);
   cv::Mat src;
...
{  split(rgb,bgr);
   ...
   inRange(bgr[2],threshold_value_red_min,threshold_value_red_max,binary_red);
   inRange(bgr[1],threshold_value_red_min,threshold_value_red_max,binary_green);
   ...
   imshow("binary_red", binary_red);
}
void binariser_blue(int,void*)
{  split(rgb,bgr);
   //threshold(bgr[0],binary_blue,threshold_value_blue,threshold_max,0);
   inRange(bgr[0],threshold_value_blue_min,threshold_value_blue_max,binary_blue);
   imshow("binary_blue", binary_blue);
}

void contour_finder(cv::Mat binary_f,Scalar identity_f, int id_t,int counter_tf)
{           
            Point pt= Point(resize_w/2,resize_h);
            circle(contours_image, pt,4, Scalar (0,255,0), CV_FILLED, 2,0); // to check the 0,0 coordinate
            ...
            for(int i=0; i<contours.size(); i++)
             ...
             { idx=id[m];
               ...
               drawContours(area_image,contours,id[m],Scalar(0,255,0),2);//, CV_FILLED);
               ...
               if (counter_tf>20)             
               {
                ...
                {       
                       ...
                       circle(contours_image, mc[idx],1, Scalar (0,255,0), CV_FILLED, 1,0);
                       
                       if(id_t==4)
                       { {putText(contours_image,"Obstacle",mc[idx], 3, 1, Scalar (0,255,0), 1);}}
                       else
                       {putText(contours_image,truck_id,mc[idx], 3, 1, Scalar (0,255,0), 1);}
                       ...
                       if (id_t==4)
                          ...
                          {  
                             ...
                             pt_left.x=left_x;
                             pt_left.y=left_y;
                             circle(contours_image, pt_left,3, Scalar (0,0,255), CV_FILLED, 2,0);                       

                             pt_right.x=right_x;
                             pt_right.y=right_y;
                             circle(contours_image, pt_right,3, Scalar (0,0,255), CV_FILLED, 2,0); 
...
int main(int argc, char **argv) {
    ...
    while (key != 'q') {
        ...
        if (zed.grab(runtime_parameters) == SUCCESS) {
            ...
            binariser_white(0,0);
            createTrackbar("binary_bar_yellow_min", "binary_yellow", &threshold_value_white_min, threshold_max, binariser_white);
            binariser_white(0,0);
            ...
            binariser_red(0,0);
            createTrackbar("binary_bar_red_min", "binary_red", &threshold_value_red_min, threshold_max, binariser_red);
            binariser_red(0,0);
           ...
           contour_finder(copy_r,identity_r,2, counter_t);
            ...
            binariser_blue(0,0);
            createTrackbar("binary_bar_blue_min", "binary_blue", &threshold_value_blue_min, threshold_max, binariser_blue);
            binariser_blue(0,0);
            ...
            imshow("tracking",contours_image);
            key = cv::waitKey(5);
